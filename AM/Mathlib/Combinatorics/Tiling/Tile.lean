...
This file defines some basic concepts related to individual tiles for tilings in a discrete
context (with definitions in a continuous context to be developed separately but analogously).
...
literature, typically when based on `ℤ`; to support the use of additive groups, `to_additive`
could be used with the theory here.
